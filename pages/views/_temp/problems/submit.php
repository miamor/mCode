<div class="files-saved col-lg-2 no-padding">
	...
	<? foreach ($myCodeFile as $mk => $myFile) {
		if (file_exists($myFile)) {
			$fs = explode('.', $myFile);
			$ext = end($fs); ?>
		<li class="list-group-item media me-file <? if ($mk == 0) echo 'active' ?>" data-id="<? echo end(explode('/', $fs[1])) ?>" data-lang="<? echo $ext ?>">
			<img style="margin:2px 4px -1px 0" class="media-object left" src="<? echo ASSETS ?>/dist/img/<? echo $ext ?>.png" width="12" height="12"> <? echo end(explode('/', $myFile)) ?>
...
<form class="code-area col-lg-10 no-padding-right">
	<div id="me-sub-code-area" class="aceditor"><? echo $myCode ?></div>
